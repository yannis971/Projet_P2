...
class Application:
	...
	def __init__(self,url):
		...
		self.runLevel = None
		p = Parser()
		parseResult = p.parseURL(url)
		# Controle de la validité de l'url passée en paramètres
		if not (parseResult and parseResult.netloc == parametres.NETLOC):
			print("impossible de lancer l'application car url invalide : {}".format(url))
		else:
		# Aiguillage en fonction de l'url passée en paramètres
	...
	def run(self):
		if self.runLevel:
			...
			print("application run level {} not defined".format(runLevel))	